...
CLAIMS: list[Claim] = [
    ...
    Claim("ostinato", "−0.00003", OST, lambda d: f"{_drift(d, 'closed_loop'):+.5f}".replace("-", "−")),
    Claim("ostinato", "−0.00078", OST, lambda d: f"{_drift(d, 'exposure_aware'):+.5f}".replace("-", "−")),
    Claim("ostinato", "±0.00020", OST, lambda d: f"±{_band(d):.5f}"),
    Claim("ostinato", "+4.76 points", OST, lambda d: f"+{100 * sum(_pairs(d)) / len(_pairs(d)):.2f} points"),
    Claim("ostinato", "6 of 6 rounds", OST, lambda d: f"{sum(1 for p in _pairs(d) if p > 0)} of {len(_pairs(d))} rounds"),
    # --- gamut -------------------------------------------------------------
    ...
    Claim("gamut", "0.936", GAM,
          lambda d: f"{min(x['artist_gini'] for x in d['frontier'] + d['artist_caps']):.3f}"),
    # --- segue -------------------------------------------------------------
    Claim("segue", "13.7%", SEG, lambda d: "{:.1%}".format(
        -sum(d["seed_counts"][k]["systems"]["segue"]["clicks"]
             / d["seed_counts"][k]["systems"]["centroid"]["clicks"] - 1
             for k in d["seed_counts"]) / len(d["seed_counts"]))),
    Claim("segue", "10.0%", SEG, lambda d: "{:.1%}".format(
        d["seed_counts"]["25"]["systems"]["segue"]["r_precision"]
        / d["seed_counts"]["25"]["systems"]["segue_shuffled"]["r_precision"] - 1)),
    # --- timbre ------------------------------------------------------------
