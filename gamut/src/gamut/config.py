...
class AuditConfig:
    ...
    n_queries: int = 400
    # Depth at which exposure is counted. A track that never enters the candidate
    # set cannot be exposed by any downstream re-ranking, so the audit measures
    # the funnel at the point where the loss actually happens.
    depth: int = 100
